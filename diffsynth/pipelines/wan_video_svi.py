...
class WanVideoSviPipeline(BasePipeline):
    ...
    def from_pretrained(
        ...
        # Initialize pipeline
        pipe = WanVideoSviPipeline(device=device, torch_dtype=torch_dtype)
        if use_usp:
    ...
    def __call__(
        ...
        inputs_shared = {
            ...
            "vap_video": vap_video, "anchor": anchor,
        }
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideoSviPipeline, input_video, noise, tiled, tile_size, tile_stride, vace_reference_image):
...
class WanVideoUnit_ImageEmbedderVAE(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("input_image", "end_image","anchor", "num_frames", "height", "width", "tiled", "tile_size", "tile_stride"),
            output_params=("y",),
            onload_model_names=("vae",)
        )

    def process(self, pipe: WanVideoSviPipeline, input_image, end_image, num_frames, height, width, tiled, tile_size, tile_stride, anchor=None,):
        ...
        msk[:, 1:] = 0
        if anchor:
            anchor = pipe.preprocess_image(anchor.resize((width, height))).to(pipe.device)
        ...
        if end_image is not None:
            ...
            if anchor is not None:
                ...
                input_pad = torch.zeros(3, num_frames-1, height, width, device=pipe.device)          
...
class WanVideoUnit_AnimatePoseLatents(PipelineUnit):
    ...
    def process(self, pipe: WanVideoSviPipeline, animate_pose_video, tiled, tile_size, tile_stride):
